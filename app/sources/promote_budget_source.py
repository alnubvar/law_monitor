...
def _build_raw_text(item: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        f"organizer: {_normalize_text(item.get('pppItemName'))}",
        f"regionName: {_normalize_text(item.get('regionName') or item.get('subjectName') or item.get('realizationPlaceName'))}",
        f"recipientCategoryName: {_normalize_text(item.get('recipientCategoryName') or item.get('recipientCategory'))}",
