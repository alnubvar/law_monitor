...
Производственная система мониторинга GR-сигналов в сельском хозяйстве:
...
сохраняет Markdown-отчёты на сервере и доставляет Telegram-сводки с DOCX-вложением.
...
- интерактивный private-only Telegram-бот с командами и reply-клавиатурой;
- архивный поиск через нижнюю кнопку `Search`;
...
Production-модель — личный диалог approved user с ботом. `TELEGRAM_CHAT_ID`
остаётся optional legacy/fallback destination для scheduled delivery и не
требуется для private-only запуска. Scheduler и Telegram-бот запускаются
...
| --- | --- |
| `/start` | открыть меню |
| `/myid` | показать Telegram ID для запроса доступа |
| `/help` | список команд |
| `/report [days]` | короткая сводка + `.docx` вложение |
| `/refresh` | ручной collect/analyze/report |

Нижняя клавиатура: `Report`, `Search`, `Refresh`, `Help`.
...
- историческая аналитика по накопленному архиву после стабилизации production;
...
- без переусложнения инфраструктуры на этапе корпоративной стабилизации.
